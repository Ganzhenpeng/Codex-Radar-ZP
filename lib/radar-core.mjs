...
function localMidnight(utcDate, timeZone) {
  const target = {
    year: utcDate.getUTCFullYear(), month: utcDate.getUTCMonth() + 1, day: utcDate.getUTCDate(),
  };
  // Use the actual IANA zone instead of assuming a fixed Pacific offset. Midnight
  // is unambiguous in Los Angeles, but a small search also keeps the helper robust
  // if the configured source time zone changes later.
  const center = Date.UTC(target.year, target.month - 1, target.day, 0, 0, 0);
  for (let minutes = -16 * 60; minutes <= 16 * 60; minutes += 15) {
    const candidate = new Date(center + minutes * 60_000);
    const parts = zonedCalendarParts(candidate, timeZone);
    if (Number(parts.year) === target.year && Number(parts.month) === target.month && Number(parts.day) === target.day && Number(parts.hour) === 0 && Number(parts.minute) === 0) {
      return candidate;
    }
  }
  return null;
...
// The Tracker's "by end of Sunday" is a latest boundary, not an exact reset
// promise.  For the compact view we expose the earliest possible boundary of
// that named local day, with an explicit third-party-estimate label in the UI.
export function earliestForecastEstimate(forecastWindow, observedAt, timeZone = TIBO_TIME_ZONE) {
  const match = /^\s*by\s+end\s+of\s+([a-z]+)\s*$/i.exec(String(forecastWindow ?? ""));
  if (!match) return null;
  const wantedWeekday = match[1].toLowerCase();
  if (!WEEKDAYS.has(wantedWeekday)) return null;
  const observed = new Date(observedAt ?? "");
  if (Number.isNaN(observed.getTime())) return null;
  const localObserved = zonedCalendarParts(observed, timeZone);
  const localDateStart = Date.UTC(Number(localObserved.year), Number(localObserved.month) - 1, Number(localObserved.day));
  for (let offset = 0; offset <= 7; offset += 1) {
    const calendarDay = new Date(localDateStart + offset * 24 * 60 * 60 * 1_000);
    const midnight = localMidnight(calendarDay, timeZone);
    if (!midnight) continue;
    const parts = zonedCalendarParts(midnight, timeZone);
    if (parts.weekday.toLowerCase() !== wantedWeekday) continue;
    return new Date(Math.max(midnight.getTime(), observed.getTime())).toISOString();
  }
  return null;
}

export function localTranscriptTranslation(text) {
  const original = String(text ?? "").trim();
  if (!original) return null;
  if (/^This celebration is moved to tomorrow as the button was already pressed today\./i.test(original)) {
    return "由于今天的重置按钮已经按下，本次庆祝活动改到明天。";
  }
  if (/^We are reseting usage for all paid users of Codex and ChatGPT Work\./i.test(original)) {
    return "我们正在为所有付费的 Codex 和 ChatGPT Work 用户重置用量。\n\n请继续阅读有关 Codex 用量限制的更新。团队一直在处理数千份报告并持续修复问题。\n\n根据你的 Codex 使用方式，用量上限相比之前可能提升约 10% 到 50%。\n\n我们逐项排查并修复了压缩时保留旧图片、后台记忆任务、目标完成后的重复运行、自定义自动化频率、子代理选择、电脑历史重复汇总、滚动任务摘要的额外请求，以及 MCP 结果重复编码等问题。\n\n我们还调整了架构以防止这些问题再次出现，并会在发生回归时通知团队；同时也在开发让你能直接看到用量去向的功能。\n\n因此我们会重置用量限制，祝你周六愉快！";
  }
  return null;
}
...
export function normalisePublicStatus(payload) {
  ...
  return {
    latestReset: latest ? {
      ...
      translationZh: localTranscriptTranslation(latest.text),
    ...
    activeWatch: watch ? {
      ...
      earliestAt: earliestForecastEstimate(watch.forecast_window, watch.observed_at),
      text: String(watch.text ?? ""), source: { kind: "tracker_transcript", author: watch.source?.author ?? "thsottiaux", url: watch.source?.url ?? null },
      translationZh: localTranscriptTranslation(watch.text),
