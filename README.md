...
- 60×60 圆形浮窗显示通用周额度剩余比例，悬停展开完整面板，鼠标离开 4 秒后收回。
...
- 浮层提供两个本机入口：“额度总览”打开 `usage.html`，“详情”打开完整雷达。用量总览只消费本机 App Server 直接提供的字段；不读取或上传线程正文，也不把未提供的绝对 token 或 eToken 伪造成数值。
